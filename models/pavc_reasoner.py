...
from pavc_reasoner.models.blip2 import Blip2Base, disabled_train
from pavc_reasoner.models.llama import LlamaForCausalLM
# from pavc_reasoner.models.qformer import BertEncoder
...
import einops
import copy
from pavc_reasoner.models.qformer import BertConfig, BertLMHeadModel
from pavc_reasoner.models.ImageBind.models.imagebind_model import ImageBindModel,ModalityType
from pavc_reasoner.models.ImageBind.models import imagebind_model

@registry.register_model("pavc_reasoner")
class PavcReasoner(Blip2Base):
    """
    BLIP2 GPT-LLAMA model.
    """
    PRETRAINED_MODEL_CONFIG_DICT = {
        "pretrain_vicuna": "configs/models/pacv_reasoner.yaml",
        "pretrain_llama_v2": "configs/models/pacv_reasoner.yaml",
    }
    @classmethod
    ...
    def __init__(
        ...
        q_former_model="/mnt/lustre/zongdaoming/models/BLIP2/blip2_pretrained_flant5xxl.pth",
        img_size=224,
        drop_path_rate=0,
    ...
    def encode_videoQformer_audiovisual(self, image, audio):
        device = image.device
    ...
    def forward(self, samples):
        if 'conv_type' in samples.keys() and samples['conv_type']=='multi':
            ...
            with self.maybe_autocast():
                # Herer maybe_autocast()as a context manager
                # 它的作用可自动将数据类型转换为模型训练时使用的数据类型，
                # float32->float16以加快计算速度并减少内存使用
    ...
    def from_config(cls, cfg):
        ...
        q_former_model = cfg.get("q_former_model", "/mnt/lustre/zongdaoming/models/BLIP2/blip2_pretrained_flant5xxl.pth")
